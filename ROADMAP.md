...
The v2.0 preview implements the first level for one selected object against the schema-validated eligible subset of the loaded browser catalog. Version 2.1 development extends the same geometric claim to bounded, durable server-side full-catalog jobs. Version 2.2 addresses catalog continuity with preferred GP/OMM ingestion and lossless six-digit identities, then adds a bounded browser-correctness follow-up for filtering, details, layering, simulation time, visual interpolation, and ephemeris limits. Version 2.3 adds SATCAT-scoped metadata discovery for tracked objects without current public GP, but does not make those records propagatable or screenable. Version 2.3.1 adds debris-oriented facets, truthful matched/positioned/metadata-only counts, object-type colors, and a guarded partial positioned-debris GP scope without claiming complete debris coverage. Version 2.3.2 improves result discoverability, direct map selection, debris-facet scope, isolated scheduled-update candidates, and artifact-only Pages controls without changing the scientific claim. Until the relevant validation gates are passed, all conjunction output remains labeled **experimental orbital-data screening**, not collision prediction or operational collision avoidance.

## Current v2.0-v2.3.2 Implementation Status

Status at 2026-09-02: **Version 2.3.2 tracked-object workflow and publication hardening is implemented in the development tree; one exact final `origin/master` source publication is approved, but it is not a candidate, stable release, or approved Pages deployment**. The authoritative product record remains `release/version.json`: channel and publication state are `development`, scientific maturity is `Experimental`, safety class is `non-operational`, and candidate/release dates are null. The source-only decision follows the warned pre-approval manifest SHA-256 `c456703d12602e83a73233f693cf684315565436d8c08c645a0b7e5d984d8177`. Before push, the authenticated GitHub Pages API changed `build_type` from legacy branch-root publishing to `workflow`; manual artifact deployment remains separate. Authoritative local verification records 63/63 JavaScript unit files, 151 passes plus one intentional skip across 152 Python cases in 83.452 seconds, a clean 49-declaration Playwright report with 29 passes and 20 intentional skips in 595,996.319 ms, 147 syntax-checked JavaScript files, zero dependency vulnerabilities, passing release engineering, and a passing 976.75 ms/95.89 MiB standalone 120,000-record guard. The validation inventory records 334 artifacts, 17 executable contracts, and 94 evidence records without self-embedding its sidecar digest. Final local static, rollback, and exact-artifact evidence passed with remote verification explicitly false. Named browser-performance profiles, remote Pages deployment/attestation, required-reviewer/self-review environment settings, clean committed-tree binding, and independent review remain pending. Nothing in this amendment approves provider completeness, collision probability, CDM/covariance, alerts, reports, maneuver work, or operational use.

| Roadmap scope | Current status | Evidence and remaining gate |
| --- | --- | --- |
| FND-1 reproducibility/CI | Implemented locally; external gate open | Locked dependencies, discovered unit/Python/browser suites, static checks, dependency audit, Version 2.1 and Version 2.2 development SBOMs, and repeatable benchmark commands exist; clean-clone evidence bound to immutable source remains required for the applicable promotion gates |
| FND-2 contracts, identity, validation | Implemented for the v2.2 catalog and v2.1 source/job envelopes | `js/domain/` now covers complete string identity, TLE/OMM/OEM/provider source formats, normalized full-catalog requests, lifecycle observations, provenance, units/frames, bounds, and job states; CDM/covariance/HBR contracts and an admitted current OMM provider remain absent |
| FND-3/FND-4 propagation and compute boundary | Implemented for browser and local full-catalog paths | Pure TLE/OMM SGP4 and multi-format propagation services, browser Worker boundary, isolated Node runner, bounded time-slab spatial engine, ADRs 0002/0003, and checksum-bound import exist; non-TEME frame conversion and higher-fidelity propagation remain open |
...
| FEAT-1 catalog diagnostics | Partially implemented | Source/provenance, path-scrubbed revisions, identity, counts, freshness, quality, and lifecycle semantics are available. Version 2.3 adds a six-component server revision, independent tracked due/status/error state, explicit current/history/propagatable/metadata-only accounting, default-off daily maintenance, and complete-only reconciliation. Normal GP/TLE upserts remain `PARTIAL`; only validated complete snapshots may infer `ABSENT`. Provider completeness, operational observation, and independent review remain open. |
| v2.2 GP/OMM and browser continuity | Development scope implemented and archived; earlier gates remain open | Preferred GP/OMM, full-string identity, compatibility enrichment, SATCAT lifecycle events, daily opt-in update/reconciliation, revision-driven refresh, truthful data health, deprecated TLE compatibility, unified browser state, batched rendering, one signed clock, current-epoch recovery, interpolation, and ephemeris limits are retained. Frozen Version 2.2.1 evidence records 56 JavaScript and 89 Python cases, 18 Playwright passes plus 9 intentional profile skips, a 122-file/205,788,178-byte static build, and 124 validation artifacts. Those exact bytes retain scoped publication approval; candidate, independent-review, performance, and rollback gates remain open. |
| v2.3 tracked-object inventory | Version 2.3.1 development scope implemented; one exact-byte publication approved; candidate gate open | The content-addressed snapshot accounts for 70,474 accepted SATCAT rows: 34,960 current plus 35,514 history-scope rows, all currently decay-dated with zero absent; 16,470 have validated current GP/OMM and 54,004 are metadata-only. All 12,490 current and 23,348 historical debris records remain metadata-only in these frozen bytes, so none is drawn. Version 2.3.1 adds tracked-object terminology, client-derived debris facets, matched/positioned/metadata-only counts, type colors, and active plus three event-specific GP groups as a guarded partial positioned subset. The 2026-08-30 live refresh returned HTTP 503 and preserved the last-known-good snapshot. Tracked server routes now reject incoherent closure/revision/current-source lineage with bounded `503 TRACKED_CATALOG_UNAVAILABLE` and retain the GP-only browser path. The manifest explicitly disclaims provider completeness and never invents positions. Owner approval covers only the final post-recording bytes published through `origin/master` and repository-root GitHub Pages. Independent review, named-hardware browser/2x profiles, long-running updates, and rollback rehearsal remain open; the rehearsal has a one-publication waiver, not completion evidence. |
| v2.3.2 catalog workflow and publication hardening | Development implementation verified; one exact source publication approved; Pages decision pending | A persistent coverage HUD and virtualized sortable results expose matched, positioned, and position-unavailable populations; keyboard navigation and narrow-viewport behavior are explicit. Positioned Globe and Mercator markers use the existing canonical selection path. Sole-object-type `DEBRIS` activates the established metadata facets for every nonempty orbit scope. Private-pointer maintenance is coherent under scheduler and request-time rebind paths. Local unit/Python/Playwright/syntax/dependency/release-engineering/120k checks pass. The validation inventory is 334 artifacts, 17 executable contracts, and 94 evidence records. Final local static-artifact, rollback, and exact-attestation evidence is recorded; remote verification is false. Authenticated repository configuration now selects the manual workflow instead of legacy branch-root publishing. Remote Pages deployment/attestation, required-reviewer/self-review environment settings, clean committed-tree binding, named profiles, and independent review remain pending. |
...
| CA-6/CA-7/CA-9 and alert/report portions of CA-8 | Not implemented and not authorized | CDM/covariance/HBR, Pc/risk policy, alert delivery, operational reporting, and maneuver what-if/recommendations remain later separately authorized work; the Version 2.2 catalog and browser-correctness scope does not include them |

Detailed method and validation limits are in `docs/science/EXPERIMENTAL_CONJUNCTION_SCREENING_V2.md` and `docs/science/EXPERIMENTAL_FULL_CATALOG_SCREENING_V2_1.md`. ADR 0004 records the GP/OMM boundary, ADR 0005 records browser state/time/motion continuity, ADR 0006 records the SATCAT-scoped tracked inventory, and ADR 0007 records revisioned private runtime candidates. Auditable switches and rollback behavior are in `release/feature-flags.json`, `docs/engineering/ROLLBACK.md`, `ROLLBACK_V2_1.md`, `ROLLBACK_V2_2.md`, `ROLLBACK_V2_3.md`, `RELEASE_CHECKLIST_V2_3_2.md`, and `ROLLBACK_V2_3_2.md`.
...
### 1.2 Recorded baseline data flow

The pre-v2.0 flow recorded by this audit was:

`CelesTrak GP/SATCAT -> satellite_data_tools.py -> flat JSON/CSV -> static fetch or Python file API -> satellite.js satrec -> Three.js`

At that baseline, the browser started rendering before deferred work, loaded the local TLE file in chunks, parsed accepted rows, created one sprite per object, and mutated the exported global `satellites` array. It checked the server after the local interface was ready and loaded server data only on reconnect. Version 2.2 superseded that path with complete mixed-catalog validation before scene replacement, preferred GP/OMM, MEO-first chunk publication, hidden-until-finite markers, and composite-revision refresh. Version 2.3 retains that scene catalog and layers a lazy metadata-only tracked index over it.

At the baseline review date:
...
- Version 2.2 supersedes the baseline update path: default preferred GP and compatibility TLE maintenance each issue one CelesTrak request for the complete `active` group. The overlapping `last-30-days` request was removed after its endpoint returned HTTP 404. These updates remain incremental/`PARTIAL`; retained or omitted records do not make the catalog equivalent to a current authoritative full space-object snapshot.

### 1.3 Runtime behavior and scaling

- At the baseline, every visible satellite was propagated in the browser animation loop.
- Mercator repeated catalog propagation at roughly 30 FPS.
- All TLE rows received individual Three.js sprites with cloned materials.
- Selected orbit paths were display geometry sampled over one period. Their samples discarded timestamps, and their geometric nearest-point helper did not establish simultaneity.
- Globe and Mercator propagation ran on the main thread, with no server-side analysis process, spatial index, conjunction job queue, or event store. The current v2.0-v2.2 Worker/service and shared-position behavior is recorded in the status table and FND-5 above.
...
- **Evidence:** Version 2.2 retains one stable `Object3D` state/selection proxy per accepted record and renders filter-visible, current-epoch-ready Globe markers through one batched `THREE.Points` layer instead of individual cloned-material sprites. Up to 1,000 drawn markers use the icon texture; denser views use compact per-marker colors. Replaced point resources are disposed under explicit loader-versus-caller material ownership. It materializes fresh-page-default `MEO` records first, publishes the first MEO-bearing chunk while catalog loading continues, and keeps every marker out of the point layer until a finite shared-motion sample covering the current simulation epoch makes it render-ready. Filter re-admission after a paused UTC jump cannot reveal an old sample; rate/selection/job changes preserve usable in-window interpolation while superseded work is discarded. It samples non-selected filter-visible SGP4 states asynchronously and Hermite-interpolates their scene positions, while the selected satellite remains exact per simulation frame and can recover at the same UTC after a bounded failure retry. Mercator reuses those positions, adapts redraw cadence, and above 1,000 drawable records replaces per-object icons/sorting/labels with one compact density path plus the detailed selected marker. It caches the selected track; a failed same-satellite rebuild clears the prior path and caches the failed instant. Propagation failures hide and retry rather than freezing stale markers. The hidden selector no longer materializes the full catalog, and the byte-identical `loral.glb` model duplicate is removed. The v2.0 conjunction screen has a dedicated Worker, but typed shared state buffers, moving propagation fully off the UI thread, and a spatial index remain open.
...
- **Implemented and recommended approach:** Retain the implemented immutable per-job revision and paginated API foundation. Next add a reviewed cross-run conjunction identity/current-event view, machine-readable and human-readable qualified reports, acknowledgement/assignment notes, and scheduled screening. Alert delivery remains later work after a separately authorized and validated likelihood/risk policy, with deduplication, escalation, quiet periods, retries, audit, and failure visibility.
- **Dependencies and prerequisites:** The local FND-6/FND-7 slice and CA-5 handoff exist. Provider retention policy, user/tenant model, report threat model, scheduling, a future validated risk policy, and notification provider remain prerequisites.
...
### Phase 3: Advanced Features (v2.1 Durable Screening and v2.2 Catalog/Browser Continuity)
...
- Scheduled screening and provider/public-redistribution admission for the Version 2.2 OMM workflow.
...
- CDM/covariance/HBR ingestion, independently validated Pc, and policy-versioned risk assessment under a separately authorized later milestone.
...
Version 2.2 development slice:

- Preferred CelesTrak GP OMM JSON without synthetic TLE conversion or identifier truncation, plus exact full-string NORAD group/tag enrichment from the one-release compatibility catalog and consistent numeric orbit reclassification.
- Mixed TLE/OMM propagation with deterministic newest-epoch deduplication and per-record quarantine.
- SATCAT-backed launch events independent of orbit availability plus confirmed-decay cache invalidation.
- Five-component server `data_revision`, daily opt-in GP/TLE/SATCAT/lifecycle update and complete reconciliation, primary `/api/gp`, data-health reporting, and deprecated TLE compatibility.
- One category-selection contract for `ALL`, `GEO`, `MEO`, `LEO`, UI-labeled `HRO`/domain `HEO`, `Debris`, and `Others`, with fresh-page-load `MEO`, union selection, debris precedence, filtered search/count parity, share-state compatibility, safe clearing when a filter excludes the selected object, and a virtualized hidden-selector bridge.
- One de-duplicated selected-satellite GP/details surface; a combined Globe/Mercator overlay above timeline/detail visualization layers; and fixed `Time x`/server controls that remain usable above Mercator-only fullscreen.
- MEO-first catalog materialization with partial-startup publication, stable per-record state/selection proxies, one density-aware batched Globe point layer, and a current-epoch finite-position render-readiness gate, plus one signed authoritative simulation clock, exact selected-satellite SGP4 state, bounded visible-catalog Hermite interpolation with failure retry/recovery, Mercator position reuse/density path/adaptive redraw/failed-track cache handling, bounded in-place selected-orbit refresh, explicit JPL ephemeris clamp/pause boundaries, and removal of duplicate clock/staggered-sprite/menu-detail paths.

Exit gate: the v2.1 portion still requires independent recall/method review, representative performance and storage budgets, clean-clone evidence, security review, and rehearsed restore/rollback. The v2.2 portion additionally requires its complete automated matrix, controlled data verification, provider/redistribution admission, coherent static packaging, and rollback rehearsal. Pc and alert gates remain tied to separately approved later releases.
...
2. Content-addressed private catalog snapshots plus startup/successful-refresh registration and deterministic new/changed/absent/reappeared semantics. Normal GP updates are incremental `PARTIAL` upserts. Only a successfully registered `COMPLETE` snapshot, including a strictly validated complete active reconciliation, may infer absence.
...
This milestone remains in development publication state. It is not promotion-complete until the v2.1 checklist's clean-clone, source licensing/admission, representative scale, independent scientific/security review, backup/restore, and rollback gates close. It did not authorize Pc/CDM/covariance work, alert delivery, or maneuver recommendations.

## v2.2 Development Milestone Authorized for Evaluation

The third development milestone is **GP/OMM Catalog Continuity with a bounded browser-correctness follow-up**, not validated collision-likelihood assessment.

It adds or requires:

1. Preferred CelesTrak GP OMM JSON ingestion with canonical fields, documented default normalization, complete string NORAD identity, newest-epoch deduplication, quarantine, provenance, atomic writes, last-known-good recovery, deterministic orbit reclassification, and compatibility-tag enrichment whose source/tag-map revisions remain auditable.
2. Mixed OMM/TLE browser propagation through the matching satellite.js APIs without fabricating TLE or silently excluding OMM from shared consumers.
3. A SATCAT-backed launch catalog independent of orbit availability, safe details-only events, and confirmed decay data under its established policy.
4. Five-component server data-revision reload for connected orbit/timeline clients, plus packaged GP-or-TLE/launch/decay revision polling in static mode without direct provider access.
5. Primary GP command/file/API contracts, temporary deprecated TLE compatibility, complete static packaging, data-health reporting, and regression coverage across unit, Python, browser, and artifact suites.
6. Unified `ALL`/`GEO`/`MEO`/`LEO`/`HRO` (`HEO` domain)/`Debris`/`Others` filtering with `MEO` as the fresh-page-load default, union behavior, object-type-aware debris precedence, compatible shared links, one filtered search/count source, and no full-catalog hidden-select option list.
7. One consolidated selected-satellite GP/details panel, removal of the duplicate menu details path, a clickable combined Globe/Mercator overlay above timeline/detail visualization layers, and fixed `Time x`/server controls above Mercator-only fullscreen.
8. MEO-first catalog materialization and partial-startup publication, with stable per-record `Object3D` state/selection proxies feeding one density-aware batched Globe point layer and filter-eligible markers withheld until a finite current-epoch position makes them render-ready; preservation of valid in-window interpolation across sampling-job changes; a single signed simulation clock for every time-dependent browser layer; exact per-frame selected SGP4 state; bounded asynchronous sampling and Hermite display interpolation for other visible satellites; stale-job rejection; finite failure retry/recovery; Mercator reuse with a compact above-1,000 density path plus adaptive redraw/track budgets and failed-track cache clearing; bounded in-place selected-orbit refresh; and no use of scene interpolation in screening.
9. Explicit Solar System ephemeris bounds that clamp and pause rather than extrapolate, allow inward reversal, preserve labeled fallback for unavailable data, and remove obsolete duplicate-clock and staggered-sprite update paths.

ADR 0004 defines the catalog architecture and ADR 0005 defines the bounded browser state/time/motion architecture. `docs/engineering/RELEASE_CHECKLIST_V2_2.md` and `ROLLBACK_V2_2.md` preserve that milestone's gates and recovery procedure. It remains Experimental and non-operational.

## v2.3 Development Milestone Implemented for Local Evaluation

The fourth development milestone is **SATCAT-Scoped Tracked-Object Discovery with GP-Only Positions**, not complete physical-debris coverage or validated collision-likelihood assessment.

It adds:

1. A deterministic tracked inventory containing every accepted row in the configured bundled SATCAT snapshot, including debris with small or missing RCS, rocket bodies, unknown types, current records without public GP, and historical/decayed records.
2. Exact canonical NORAD joins to the configured `active` GP/OMM snapshot. Only newest-epoch validated GP records are propagatable; SATCAT-only records remain searchable details and never receive synthetic orbital state.
3. Independent object type, orbit class, lifecycle, observation-transition, element-availability, history-scope, and tag semantics. RCS remains provider metadata and is never translated into diameter.
4. An atomic content-addressed manifest with verified current/history/quarantine chunks, explicit source revisions, complete local accounting invariants, `expected_provider_records: null`, and `provider_completeness_claim: false`.
5. GP-priority `MEO` startup followed by lazy type/history chunks, bounded results, generation-based stale-request rejection, a normalized identity cache, eight population diagnostics, and details-only metadata selection. Selected-object screening summaries and exports also disclose the current tracked, propagatable, and metadata-only-excluded input populations without presenting that ratio as pair-screening completeness.
6. Manifest and allowlisted chunk APIs, a sixth tracked component in data health/revision state, and an independent daily scheduler clock that rebuilds from the coalesced local SATCAT/GP inputs without another provider request.
7. Static packaging of only the verified current manifest closure, large-catalog budgets, real-snapshot invariants, 120,000-record synthetic filter/search coverage, failure/race/security tests, rollback, and checksum-bound development evidence.

The frozen local snapshot has 70,474 accepted records, split into 34,960 current and 35,514 `history_total`; all history-scope records are currently decay-dated and `absent` is zero. It has 16,470 propagatable records and 54,004 metadata-only records; 18,490 metadata-only records are current. These numbers describe one bundled snapshot, not a provider guarantee. ADR 0006 defines the boundary. `docs/engineering/RELEASE_CHECKLIST_V2_3.md` and `ROLLBACK_V2_3.md` define the open gates and recovery procedure.

Version 2.3 remains Experimental, non-operational, and development-only. The Version 2.2.1 approval does not authorize publication of the newly derived manifest, metadata, quarantine, chunks, validation manifest, or static artifact. This milestone does not close earlier gates or authorize provider-completeness claims, Pc/CDM/covariance, operational risk, alerts, reports, or maneuver recommendations.

### Version 2.3.1 Amendment

Version 2.3.1 keeps the Version 2.3 tracked schema and manifest/chunk API unchanged. Only exact orbit `ALL` plus object type `DEBRIS` activates the client-derived position availability, reported-RCS, owner/country, launch-year, launch-site, provider-status, and designator/tag facets; ordinary orbit/object/history/tag filtering remains independent outside that special workflow. No mass or weight inference or filter is permitted. The interface uses tracked-object language and separates matched, positioned, and metadata-only counts. A compact color key maps payload to cyan, debris to red, rocket body to amber, mission-related to green, unknown to gray, and selected to white. Red denotes authoritative debris type only, never hazard, collision risk, proximity, physical size, mass, or RCS.

For 0 through 499 render-ready/drawn objects, the Globe renders the bundled same-origin `icons/ob_satellite.png` alpha silhouette at a fixed 16 screen pixels with `sizeAttenuation: false` and applies the exact canonical vertex color. At 500 it removes the map, restores perspective attenuation, and uses compact density size `0.025`; the procedural white-alpha circle is a load-failure fallback only. Mercator keeps its separate detailed/density threshold above 1,000 drawable objects.

The configured GP scope now comprises `active`, `fengyun-1c-debris`, `iridium-33-debris`, and `cosmos-2251-debris`. The three event groups are only a partial positioned-debris subset. Complete reconciliation requires a valid, quarantine-free response from every configured group and never establishes provider-universe completeness. Metadata separates configured `source_groups` from accepted-byte `catalog_source_groups`. The `2026-08-30T20:33:29Z` live refresh returned HTTP 503; fail-closed promotion preserved the last-known-good GP bytes/revision, kept actual catalog scope at `[active]`, and left the configured scope unverified and retry-eligible without inherited validators. A later tracked provenance correction retained the same tracked catalog revision, chunks, and counts. Its 12,490 current and 23,348 historical debris records all remain metadata-only, so the real frozen scene contains zero positioned debris even though future validated debris elements will render red.

Tracked API routes and their `server.py` static aliases validate the complete manifest/metadata/chunk closure, pointer revision agreement, and current GP/SATCAT lineage as one publication. Incoherent state returns bounded `503 TRACKED_CATALOG_UNAVAILABLE`, degrades health, and retains the browser's GP-only fallback rather than exposing a partial tracked index. Plain static deployments remain dependent on the curated builder's frozen-closure validation.

### Version 2.3.2 Amendment

Version 2.3.2 keeps the tracked Version 2.3.0 manifest/chunk schema and the GP-only position boundary. It adds a persistent visualization-side coverage HUD for matched, positioned, and position-unavailable records, including current/history scope, catalog state, and an available generation timestamp. A virtualized sortable results drawer exposes `All matches`, `On map`, and `Position unavailable` views without materializing an unbounded DOM list; keyboard navigation, focus restoration, and narrow-viewport reflow are part of the development contract. Positioned Globe and Mercator markers can be selected directly and must resolve through the existing canonical selection/details path. Metadata-only records remain details-only.

The Version 2.3.1 debris facets now apply whenever `DEBRIS` is the sole selected object type and at least one orbit scope is selected, including LEO, MEO, GEO, HRO/HEO, Other, and ALL. Orbit, history, tag, and facet dimensions remain independent and share-state restorable. Reported RCS remains provider metadata; the project neither supplies nor infers a mass/weight field. Red still means authoritative debris object type only.

Opt-in scheduled maintenance now creates an isolated revisioned candidate beneath a private runtime boundary, updates only that candidate, validates every required GP/TLE/SATCAT/launch/decay revision pair plus the complete tracked closure and current GP/SATCAT lineage, and replaces one private current-candidate pointer only after success. Candidate writers share a persistent regular file held by a nonblocking cross-platform OS advisory lock; neither contention nor stale owner text permits unlink or reclamation. Cooperative cancellation, validation failure, degraded state, source drift, or pointer-write failure leaves the prior pointer and checked-in release data unchanged. Data routes resolve from one registered coherent root and retain the checked-in closure as fallback when no valid pointer exists. Default `serve` rechecks the verified pointer on mutable data and `/api/v1` requests, bootstraps or transactionally activates V21 before committing a changed root, retains the prior root on failure, and retries on the next qualifying request; A to B to A can reactivate an existing revision. Only scheduler-path registration failure appears in public data-update status, leaves the successful pointer selected with registration pending, and retries on the next scheduler pass. ADR 0007 defines this single-node integrity boundary; restart remains the recommended rollback procedure, and none of this is a hosted ingestion or operational-data-service claim.

Two pinned no-network imports record the recovery without selecting a private pointer. Exact-commit candidate `20260902T014734Z-9faea7cdb724` is `VALIDATED` and deliberately unpromoted, not quarantined; its 24-artifact/131,613,453-byte raw closure has zero source-copy mismatches and tracked revision `sha256:8fd7f619f16e714a9d170a8eb538a183e240051830430b09b1201bbf0d36e4a4` reports 70,475 total, 19,111 propagatable, 12,488 current debris, and 2,640 positioned GP debris. The distinct dirty-runtime candidate `20260902T011924Z-8c8d999bf6df` is `QUARANTINED` for malformed TLE metadata; its 24-artifact/131,692,789-byte import also recorded zero copy mismatches and tracked revision `sha256:de60852484cc6ccee624380286080b993543ad6c412016710673ccd1b75f4cb7` reports 70,532 total, 19,106 propagatable, 51,426 metadata-only, 12,488 current debris, and 2,640 positioned GP debris. The checked-in 12-file set remains restored from strict last-known-good `ef98cfe`; revision `sha256:7c1a20d93d1eb5faf7e2b964b13c7b4f0478f2eec95cc701ea1b1e57ef0d730c` reports 70,474 total, 16,470 propagatable, 54,004 metadata-only, 12,490 current debris, and zero positioned debris. Validation, quarantine, pointer selection, and release approval are separate decisions.

Pages publication is now designed as a manual, artifact-only path bound to one confirmed `master` commit. Release-tree verification, deterministic `dist/` construction, a disposable rollback rehearsal, evidence archival, artifact deployment, and deployed-byte attestation are required technical controls. Authenticated repository configuration changed Pages `build_type` from legacy branch-root publishing to `workflow` before the source push. Repository environment protection and reviewers remain external settings. Local unit/Python/Playwright/syntax/dependency/release-engineering checks, the standalone 120,000-record observation, source-byte ceilings, a 334-artifact/17-executable/94-evidence validation inventory, and final local rollback/exact-artifact verification are recorded, while remote verification is false. Version 2.3.2 stays Experimental, non-operational, and in development state. Owner `arcazj` approved one exact final post-recording source publication to `origin/master`; the decision does not approve manual Pages deployment or later changed bytes. Remote deployment/attestation, required-reviewer/self-review settings, clean committed-tree binding, named profiles, and independent review remain pending.
