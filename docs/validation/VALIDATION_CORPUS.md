...
The checksum-consistent v2.0 candidate manifest is `validation/v2.0.0/manifest.json`, with its own sidecar digest, the official AIAA-2006-6753 package checksum, and SHA-256 checksums for the executable fixture files. The historical v2.1 and v2.2.0 manifests retain their original source hashes and immutable benchmark/evidence records while allowing current source files to be superseded by v2.2.1. The current `validation/v2.2.1/manifest.json` binds GP/OMM ingest and generated data, five-component server revisions, daily update/reconciliation and recovery contracts, same-origin static refresh, immutable registry acquisition/fallback, API, benchmarks, Python discovery, offline static closure, browser state/time/motion/rendering, packaging, and tests. Its final inventory contains 124 hashed artifacts and 46 named evidence records, including byte-exact Windows CRLF SATCAT revision/no-churn coverage. `npm run check:validation` verifies all four sidecars, rejects drift in current v2.2.1 artifacts, requires named executable/evidence records, enforces obsolete-asset removal, and preserves pending independent-review states. Coordinated edits remain possible until publication under an immutable tag, archive, or signature.
