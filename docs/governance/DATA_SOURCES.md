...
The current private registry identifies the preferred Version 2.2 OMM instance as `celestrak-gp-omm-bundled`; `celestrak-gp-bundled` remains the legacy TLE fallback ID. These distinct runtime IDs do not imply different providers. Local implementation and retrieval do not by themselves approve public redistribution; the scoped owner approval below is the controlling record for the checked-in Version 2.2.1 snapshot.
...
| Update policy | The local tool enforces a two-hour minimum between successful refreshes unless explicitly forced and sends conditional request metadata where available. Preferred GP and default incremental TLE compatibility updates each request the CelesTrak `active` group once; the prior overlapping/failing `last-30-days` incremental request is removed. Explicit `export-tle --all` retains the historical multi-group compatibility workflow. An accepted `304` revalidation advances successful freshness and resets the due age without replacing data. The tool does not claim to know the provider's publication boundary. |
| Registry completeness | Successful incremental metadata is registered as `PARTIAL`; only a successful full/reconciliation snapshot is `COMPLETE`. For an established prior GP, TLE, or SATCAT catalog of at least 1,000 records, a full replacement must have both candidate size and canonical NORAD overlap at or above 75% of the prior catalog. Failed, unrecognized, or guard-rejected modes are `DEGRADED`. |
| License and redistribution | CelesTrak describes the data as freely available. Repository-owner confirmation dated 2026-08-29 records data-owner approval to redistribute the exact Version 2.2.1 bundled snapshot through the repository-root GitHub Pages publication. Future changed snapshots remain subject to renewed provenance and release review. |
| Retention | Atomic last-known-good GP JSON plus fetch metadata, SATCAT-derived launch/decay artifacts, and the deprecated TLE compatibility files. Changed artifacts use collision-safe timestamped backups with the newest seven retained per artifact; byte-identical or revalidated data creates no backup. A coherent promoted revision must not mix old data with new metadata. The local durable registry freezes the catalog bytes, first source metadata supplied for that catalog byte hash, and revision descriptor; distinct later acquisitions of identical catalog bytes are append-only records and do not rewrite those frozen files. |
| Integrity and validation | HTTPS, provider ETag/Last-Modified when supplied, release-evidence SHA-256 hashes, OMM schema/identity/epoch/frame/theory/range validation, TLE compatibility structure/checksum validation, deterministic newest-epoch deduplication, exact-string enrichment joins, deterministic tag-map/source revisions and match counts, numeric orbit reclassification, record quarantine, freshness/count diagnostics, and the production-scale dual size/identity replacement guard |
| Freshness and outage | Source metadata records last attempt/success/revalidation. An accepted not-modified response resets due age; network, parse, or shrink-guard failure preserves the prior dataset and exposes degraded/partial state. GP, TLE, SATCAT, launch, and decay sidecars retain their latest dataset error for restart-safe status. |
| Owner | Project maintainers; no external service-level agreement |

**Coverage restriction:** CelesTrak reports that the official catalog crossed into six-digit catalog numbers in July 2026. A TLE catalog field can encode a limited Alpha-5 subset, which Version 2.2 decodes to canonical full numeric strings, but it cannot provide complete current six-digit coverage. The preferred OMM profile and launch catalog preserve non-fixed-width identifiers. That closes the application-format blocker only; provider completeness, SATCAT profile compatibility, and independently verified current coverage remain open gates. The scoped redistribution approval above is not a completeness or scientific-validation claim.
...
`/api/gp-metadata` exposes the generated `json/gp/GP.meta.json` sidecar unchanged. `/api/data-update-status` publishes a deterministic server `data_revision` over GP, compatibility TLE, SATCAT, launch, and decay revisions, exposes every component directly and under `datasets`, and retains GP-only `catalog_revision` for compatibility. It reconstructs all five dataset histories from their metadata after restart, merges live results, and recursively bounds, normalizes, and credential-redacts public nested error text. Consumers connected to the server should use `data_revision` for cross-dataset refresh detection. Static deployments compute a separate packaged-file token because they have no mutable provider state.
...
| `CCSDS_OMM_JSON` | Requires canonical OMM SGP4 fields, normalizes only the documented CelesTrak omitted constants, preserves complete string identity and original JSON, and propagates through `json2satrec` | Preferred Version 2.2 local-development GP profile; current Version 2.2.1 bundled bytes have scoped redistribution approval, while completeness and future-refresh review remain open |
...
| SATCAT enrichment and launch catalog | Builds an unambiguous identity index, preserves launch/object/lifecycle metadata, and emits details-only events independently of orbit availability | Current Version 2.2.1 bundled SATCAT-derived bytes have scoped redistribution approval; exact profile compatibility, completeness, and future-refresh review remain open |
...
For bundled catalog registration, Version 2.2 retains the Version 2.1 `NEW`, `CHANGED`, stable observed, `ABSENT`, and `REAPPEARED` semantics by stable object/element identity. Normal scheduled GP/TLE updates are incremental `PARTIAL` upserts and never generate `ABSENT`. A scheduled reconciliation may emit absence only after a structurally valid, quarantine-free, complete CelesTrak `active` response is promoted as `COMPLETE`; explicit full (`mode=all`) snapshots remain complete-source evidence for their configured scope. If the prior catalog has at least 1,000 records, the candidate must also retain at least 75% of its record count and 75% of its canonical NORAD identities. The same dual guard protects SATCAT replacement. Failed, partial, conditional-without-matching-revision, guard-rejected, or unrecognized source modes must not turn missing records into absence. Authenticated catalog API documents omit private snapshot and metadata paths.
...
Provider maintenance is disabled unless `--update-data-on-schedule` or `npm run serve:update` is selected. The daily profile evaluates GP, compatibility TLE, SATCAT, launch, confirmed-decay, and reconciliation clocks independently, coalesces SATCAT-derived work into one provider request per cycle, and keeps historical launch/decay events when a later SATCAT snapshot omits them. Content-identical results do not rewrite data or create backups; accepted conditional revalidation advances freshness so it is not immediately due again. Failures preserve last-known-good artifacts and retry with bounded jittered backoff; a dead or expired single-writer lock can be recovered. Scheduled maintenance always enforces the dual shrink/identity guard: `--force` does not bypass it, and the explicit direct-command `--allow-large-catalog-shrink` option is intentionally absent from `maybe-update` and `server.py`. A component revision changes only after its bytes change successfully, so the server composite revision can advance without an unrelated GP promotion. GitHub Pages and other static hosts cannot run this scheduler.
