...
- Keep `runtime/` on private local storage with enough capacity for the SQLite database, immutable catalog copies, isolated scheduled-update candidates, pointer metadata, runner inputs, and result artifacts.
...
Cancellation, source drift, rejected validation, degraded updater state, process interruption, or pointer-write failure leaves the previous pointer intact. Graceful shutdown requests cooperative cancellation before publication and waits for the non-daemon scheduler worker. API and `server.py` static data routes resolve from the same registered coherent root and use the checked-in closure only when no valid private pointer exists. Private candidate paths and pointer contents are never API fields.

The default server rechecks the fully verified pointer before mutable data and `/api/v1` requests, even when scheduled maintenance is disabled. When the pointer identity changes, one request-time coordinator first bootstraps or activates V21 against the selected candidate and only then commits the new registered root and identity. A failure restores and serves the prior coherent V21/data root, does not publish a coordinator error through `/api/data-update-status`, and retries on the next qualifying request. Returning an existing pointer from A to B to A transactionally reactivates revision A and its observations rather than treating its prior registration as sufficient.

The scheduler calls the same coordinator after a promotion. Before provider refreshes, it can also repair stale tracked lineage from the accepted local GP/SATCAT bytes in a separately validated candidate. A successful local repair remains selected if the provider candidate is rejected, and the cycle result reports `local_repair_promoted`. Source hashes, tracked history, and the complete candidate must still pass validation; provider freshness and source coverage stay unchanged. If the scheduler callback fails transiently, the already-promoted pointer is not rolled back: the scheduler reports a degraded cycle through `/api/data-update-status`, keeps V21 registration pending, and retries on the next scheduler pass even when the data update itself is skipped and no new promotion occurs. A successful retry clears the pending state and scheduler error. Restart remains the recommended operator rollback procedure despite the live request-time rebind path. A local promotion is not repository publication, provider admission, or release approval; candidate retention, disk pressure, long-running updates, and prior-pointer restoration remain release gates.
