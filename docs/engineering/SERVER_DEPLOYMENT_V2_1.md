...
4. Reconciles current observations as new, changed, observed, absent, or reappeared. A bundled revision takes `COMPLETE`, `PARTIAL`, or `DEGRADED` from the first metadata frozen for that catalog byte hash. Scheduled GP refreshes are incremental and therefore `PARTIAL`; only a successfully registered `COMPLETE` full (`mode=all`) snapshot may emit absence.
...
When scheduled data maintenance is explicitly enabled, GP, launch, and confirmed-decay freshness are evaluated independently. Launch and decay work can both execute during the same scheduler cycle; neither due branch suppresses the other. A successful GP promotion invokes the same private snapshot/registration path before later jobs use the new current catalog. Any successfully promoted GP, launch, or decay component changes the composite `data_revision` used for browser catalog/timeline refresh, while `catalog_revision` remains the GP-only compatibility field. The scheduler is incremental and therefore registers GP updates as `PARTIAL`; it cannot mark missing objects absent. Failed, malformed, partial-promotion, or unchanged provider operations retain their truthful source state and do not fabricate a complete revision.
...
`/api/gp-metadata` returns the generated `json/gp/GP.meta.json` sidecar unchanged and returns `404` if it has not been exported. `/api/data-update-status` returns the composite `data_revision`; the compatibility GP-only `catalog_revision`; `gp_revision`, `launch_revision`, and `decay_revision`; and the same values below `datasets`. When GP metadata is absent, a packaged TLE file larger than the empty `[]` sentinel yields `fallback-tle`; if no such artifact exists, `catalog_state` is `unavailable`. With GP metadata present, normal `current`/`partial`/`degraded` evaluation applies. The fallback status is an artifact-availability check, not TLE parse or propagation validation.
...
- Scheduled screening, report export, notifications, and operator workflow are not included. GP/SATCAT maintenance is optional, guarded, and disabled by default.
...
Use `ROLLBACK_V2_1.md` for durable-service disablement and restoration. Use `ROLLBACK_V2_2.md` for GP/launch/decay revision rollback. Both version checklists remain open before any candidate decision.
