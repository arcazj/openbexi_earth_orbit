...
Version `2.2.0` is authorized only as a `development` build with `Experimental` scientific maturity and a `non-operational` safety class. It is not a release candidate or release. This checklist does not close the v2.0 or v2.1 gates, admit a provider for validated/public redistribution, or authorize Pc/CDM/covariance, alerts, reports, or maneuver work.

## Development Scope

- [x] `release/version.json`, package metadata, feature-flag metadata, and generated browser metadata identify Version 2.2.0 development with null candidate and release dates.
...
- [x] `/api/data-update-status` reports composite `data_revision`, GP-only compatibility `catalog_revision`, top-level and nested per-dataset revisions, retrieval time, newest orbital epoch, launch and confirmed-decay dates, TLE/OMM/six-digit/quarantine counts, and the last error without presenting partial failure as complete; `fallback-tle` remains explicitly an artifact-availability signal rather than validation.
...
- [x] OMM defaults, malformed JSON/identity/epoch/numeric/frame/theory quarantine, duplicate epoch choice, 404 handling, conditional requests, dry run, atomic write, backup, partial failure, metadata, and last-known-good preservation are covered.
...
- [x] GP, launch, and decay scheduler due work is independent, including simultaneous launch and decay work without an unrelated GP refresh.
...
- [x] The packaged-GP Chromium smoke gate reaches finite default MEO markers in less than `12 s`, full catalog readiness in less than `20 s`, validates exact point-buffer membership above 16,000 records, and renders a nonblank Mercator density canvas within its declared frame-gap bounds.
...
- [ ] Dataset/derived-artifact redistribution terms, attribution, cadence, retention, and outage behavior are approved by the data owner before any public artifact publication.
...
Current decision: **Version 2.2 development is authorized; candidate, release, validated, public-redistribution, and operational decisions remain open**. Every unchecked item above remains a blocker unless a named owner records a reviewed waiver with scope, evidence, expiration, and rollback impact.
