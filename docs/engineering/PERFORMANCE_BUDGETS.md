...
`release/asset-budgets.json` separately caps the complete manifest-referenced tracked closure at `82,000,000` bytes and each content-addressed chunk at `27,000,000` bytes. The Version 2.3.0 closure measured `74,565,443` bytes across 13 files. The frozen Version 2.3.1 referenced closure measures `74,565,511` bytes across 13 files, leaving `7,434,489` bytes of aggregate headroom; its largest chunk remains `24,215,754` bytes, leaving `2,784,246` bytes of per-chunk headroom. Version 2.3.2 deliberately restores this strict checked-in last-known-good closure and therefore has the same 13-file, `74,565,511`-byte referenced closure and `24,215,754`-byte largest chunk. The exact-commit private import is validated but unpromoted; the distinct dirty-runtime import is quarantined. Neither is selected or included in the static artifact. The checker follows only the active manifest references, so retained orphan or rollback chunks do not silently consume the publication budget.
...
## v2.3.2 Interaction Development Gate

The Version 2.3.2 browser source budget retains the `315,000`-byte `index.html` ceiling; the refactored entrypoint measures 314,194 bytes, leaving 806 bytes of headroom. Modular first-party browser JavaScript measures 904,188 bytes across 54 files against a reviewed `915,000`-byte ceiling, leaving 10,812 bytes. The JavaScript increase covers the persistent coverage HUD, virtualized result modules, direct map-selection paths, accessibility/tablet behavior, and the fail-closed, byte-preserving catalog parser plus cross-runtime record validation required by the release trust boundary. These unminified source measurements are regression ceilings, not compressed transfer, parse/startup time, runtime heap, static-artifact total, final byte seal, or performance approval.

Version 2.3.2 extends the deterministic 120,000-record Node guard through the virtualized results workflow. In addition to the retained 2.5-second build, 3-second filter, 3-second search, 6-second facet, and 384 MiB heap-growth limits, the benchmark must prepare the selected results ordering and compute a bounded virtual window in `resultsMs <= 3,000 ms`. The combined build/filter/search/facet/results limit is `totalMs <= 16,000 ms`. The benchmark asserts that the logical availability view retains the full matched count while the returned DOM-oriented window remains bounded by viewport, row height, and overscan inputs.

The authoritative Version 2.3.2 standalone 120,000-record observation measured 57.56 ms build, 245.79 ms filter, 53.86 ms search, 288.86 ms facet construction, 330.68 ms result ordering/window preparation, 976.75 ms total, and 95.89 MiB heap growth. It passed every retained component ceiling plus the new 3-second results and 16-second total ceilings. No Version 2.3.1 timing is carried forward as a Version 2.3.2 observation. Exact final static-artifact counts, bytes, and digest are recorded in `dist/asset-manifest.json`, `release/evidence/v2.3.2-release-metrics.json`, and the validation manifest rather than duplicated here. Named browser profiles remain pending; before any candidate decision, those browser profiles must measure drawer-open/sort/tab latency, scroll responsiveness, map hit-test latency in detailed and density modes, narrow-viewport behavior, and the cost of keeping the coverage HUD current during rapid filter-generation changes. Passing the synthetic Node, source-byte, or static-byte ceilings is a regression guard, not production-capacity or accessibility evidence.
