...
> Current application context: Version 2.3.2 reuses this durable-service boundary with preferred GP/OMM catalog input. Its four-group provider/update scope, mixed-format validation, tracked inventory, revisioned private scheduled-update candidates, lifecycle cache, static packaging, orbit-scoped debris facets, result/picking workflow, and browser state/time/motion risks are covered by `THREAT_MODEL_V2.md`, ADRs 0004 through 0007, and `RELEASE_CHECKLIST_V2_3_2.md`. Metadata-only tracked records do not enter durable screening. This historical v2.1 model does not grant provider admission, redistribution, publication, or operational approval.

Last reviewed: 2026-09-01

## Scope and Trust Boundary

This model covers the development `/api/v1` service, private runtime directory, scheduled-data candidates and pointer, SQLite store, catalog snapshot registry, Node screening subprocess, browser client, and SSE connection. It supplements the v2.0 browser/static model in `THREAT_MODEL_V2.md`.
...
- revisioned scheduled-update candidates, their validation evidence, and the private current pointer;
...
| Candidate substitution or mixed generations | Candidate root confinement and symlink rejection, complete data/metadata revision-pair and tracked closure/lineage validation, atomic pointer replacement, cancellation/drift check, immutable checked-in fallback, one registered coherent request root, verified-pointer recheck on mutable and `/api/v1` requests, V21 bootstrap/transactional A to B to A activation before root commit, coherent fallback plus next-request retry without public scheduler-error exposure, one persistent regular lock file held by a nonblocking Windows/POSIX advisory lock with no unlink/reclaim takeover, scheduler-visible pending retry after transient scheduler-path post-promotion registration failure, and no public candidate endpoints | Single-host filesystem compromise, disk exhaustion, retention policy, long-running provider rehearsal, and independent security review remain open |
...
Any catalog upload, public candidate management, remote ingestion, report rendering, webhook, alert, or CDM endpoint requires a new threat-model review. Version 2.1 implements none of those public surfaces.
