...
- `tools/satellite_data_tools.py` uses HTTPS, conditional requests bound to a matching reconciled revision, minimum intervals, content-aware atomic writes, a stale-owner-aware single-writer lock, quarantine, and last-known-good preservation. Normal GP/TLE work is non-pruning. For any established GP, TLE, or SATCAT catalog with at least 1,000 records, a complete/full replacement must retain at least 75% candidate size and 75% canonical NORAD overlap. `--force` bypasses freshness only; the explicit `--allow-large-catalog-shrink` recovery override exists only on direct export/refresh commands and is absent from unattended `maybe-update`/server paths. Complete reconciliation otherwise uses one `active` request per orbital format and prunes only after structural and record validation; one SATCAT fetch feeds launch/decay upserts without deleting history. Accepted `304` revalidation resets due age without changing bytes/revision. Changed artifacts receive collision-safe backups with the newest seven retained per artifact. Temporary tag enrichment is a full-string NORAD join. Scheduled access is default-off, credential-gated fallbacks remain disabled, per-dataset failures are isolated, and bounded retry prevents tight failure loops. Static hosts cannot execute the scheduler.
...
- Residual risks: provider compromise, very large compressed responses before parsing, semantic but structurally valid bad data that retains the size/identity thresholds, SATCAT/OMM identifier conflicts, intentional misuse of the direct shrink override, stale client/CDN revisions, incomplete provider coverage, and accidental publication of future refreshed bytes outside the scoped 2026-08-29 data-owner approval.
...
- `server.py` defaults to loopback, requires explicit public binding, restricts CORS, validates `Host`, resolves and confines paths, blocks traversal and symlink escape, and serves only exact root/runtime allowlists with controlled content types, cache behavior, and security headers. Data-update status reconstructs persisted GP/TLE/SATCAT/launch/decay errors after restart and recursively bounds, control-character normalizes, and credential-redacts all public nested result strings.
