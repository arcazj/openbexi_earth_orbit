...
- `js/conjunction/conjunctionScreening.js` compares primary and secondary states at identical UTC instants. Scene coordinates, rendered/in-place-refreshed orbit lines, cached Mercator ground tracks, per-record state proxies, batched/density marker buffers, marker radii, and visual scale are never screening inputs. Version 2.2 may Hermite-interpolate non-selected proxy positions between bounded asynchronous SGP4 display samples and reuse those positions in the Globe point layer and Mercator, but that interpolation remains outside the Worker calculation; the selected display object continues to use an exact per-frame SGP4 state. Hiding/retrying a failed visual propagation does not change the Worker's explicit error/result state.
