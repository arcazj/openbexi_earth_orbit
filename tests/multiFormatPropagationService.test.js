...
const tlePrepared = service.prepareObject(tleRecord);
assert.strictEqual(tlePrepared.ok, true, tlePrepared.error?.message);
assert.strictEqual(tlePrepared.value.route, 'TLE');
assert.strictEqual(tlePrepared.value.object_id, tleRecord.object_id);
assert.strictEqual(tlePrepared.value.element_set_id, tleRecord.element_set.element_set_id);
assert.strictEqual(tlePrepared.value.delegated.object_id, tlePrepared.value.object_id);
assert.strictEqual(tlePrepared.value.line1, tleRecord.element_set.line1);
assert.strictEqual(tlePrepared.value.line2, tleRecord.element_set.line2);
assert.strictEqual(tlePrepared.value.element_epoch_utc, tleRecord.element_set.epoch);
assert.strictEqual(tlePrepared.value.orbit_class, tlePrepared.value.delegated.orbit_class);
assert.strictEqual(tlePrepared.value.dataset_id, tleRecord.provenance.dataset_id);
assert.strictEqual(tlePrepared.value.dataset_hash, tleRecord.provenance.dataset_hash);
assert.deepStrictEqual(tlePrepared.value.input_quality_flags, tlePrepared.value.delegated.input_quality_flags);
assert.deepStrictEqual(tlePrepared.value.quality_flags, tlePrepared.value.delegated.quality_flags);
assert.strictEqual(tlePrepared.value.satrec, tlePrepared.value.delegated.satrec);
