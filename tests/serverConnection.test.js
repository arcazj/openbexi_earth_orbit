...
async function run() {
  ...
  const connected = await checkServerConnection({
    ...
    fetchImpl: async (url) => {
      ...
      if (url.endsWith('/api/version')) return response(true, { api_version: '1.5.22' });
      return response(false, {}, 404);
    }
  });
  assert.strictEqual(connected.state, 'connected', 'health ok marks server connected');
  assert.strictEqual(connected.version.api_version, '1.5.22', 'version payload is captured');
