...
Version 2.3.1 remains Experimental, non-operational, and in development publication state. Repository owner `arcazj` approved the exact final post-recording tracked/static/validation/source bytes for one commit/push through `origin/master` and repository-root GitHub Pages publication. That approval does not cover future changed or refreshed bytes and does not constitute scientific validation. Automated provider tests use fixed fixtures, local frozen inputs, injected clocks/jitter, or mocked fetches; CI must not depend on live CelesTrak availability.

### Source accounting and publication

- Build from local `json/satcat.csv` plus accepted `json/gp/GP.json`; assert `build-tracked` performs no provider request. Preserve every accepted SATCAT row, including `PAYLOAD`, `DEBRIS`, `ROCKET_BODY`, `MISSION_RELATED`, `UNKNOWN`, small RCS, and missing RCS.
- Assert `received == accepted + quarantined + duplicates`, `total == current + history_total`, `total == propagatable + metadata_only`, and `current == current_propagatable + current_metadata_only`. `historical` counts decay-dated records and `absent` counts identities removed from current membership; each is bounded by `history_total`, but they can overlap and are not added to derive it. Reconcile every manifest descriptor against chunk bytes, SHA-256, record count, type, scope, and unique canonical NORAD membership.
- The frozen real-snapshot regression must require 70,474 accepted rows, 34,960 current, 35,514 `history_total`, 35,514 decay-dated `historical`, zero `absent`, 16,470 total/current propagatable, 54,004 total metadata-only, and 18,490 current metadata-only only in checksum-bound development evidence. Behavioral tests should prefer invariant/minimum/representative assertions so a reviewed future provider snapshot can change counts deliberately.
- Verify current types include payload, debris, rocket body, and unknown; mission-related remains zero unless the source supplies that classification. Verify representative current debris includes published RCS below 0.1 m2 and missing RCS. The frozen snapshot has 12,490 current and 23,348 historical debris records, all metadata-only with `NO_CURRENT_ELEMENTS`, and therefore zero positioned debris. `rcs_m2` remains provider-reported radar cross-section; no mass, weight, diameter, physical-size, hazard, or visibility decision is inferred from it or `physical_size_estimate`.
- Preserve numeric IDs of arbitrary supported length and Alpha-5 evidence as strings. Resolve duplicate SATCAT identities deterministically and report the duplicate rather than silently increasing coverage.
- Keep lifecycle, catalog membership, observation transition, element availability, object type, and orbit class independent. A complete reconciliation may mark a previously retained omission `ABSENT`; incremental input, failure, or partial input may not.
- Assert `expected_provider_records` is null and `provider_completeness_claim` is false even when the local source accounting is `VERIFIED_SNAPSHOT` and `complete_source_snapshot` is true.
- Write and validate all content-addressed chunks before atomically promoting `TRACKED.manifest.json`. Exercise interrupted writes, corrupt chunks, wrong scope/type, duplicate identity, count/hash/byte mismatch, missing input, failed promotion, no-change input, and last-known-good restoration. A missing SATCAT or another pre-build failure must persist a bounded failed/`DEGRADED` sidecar across restart without replacing the last successful revision, counts, or reconciliation time. No-change/revalidation must not churn data or backups.

### Filters, search, counts, and selection

- A fresh page uses `MEO` orbit, all object types, current-only scope, and no restrictive tag or facet. Default GP/MEO interaction must not wait for the tracked manifest or chunks.
- Orbit values are `ALL`, `GEO`, `MEO`, `LEO`, `HEO` (visible `HRO`), and `OTHER`. Object types are a separate `ALL`, `PAYLOAD`, `DEBRIS`, `ROCKET_BODY`, `MISSION_RELATED`, and `UNKNOWN` dimension. Values within one dimension form a union; dimensions intersect.
- Selecting orbit `MEO` must never admit a `GEO` record merely because its tag or object type matches. Selecting orbit `MEO` plus type `DEBRIS` shows only MEO debris. Repeat the same membership assertion for `GEO`, `LEO`, `HRO`/`HEO`, and `OTHER`.
- Selecting orbit `ALL` with type `ALL` shows every loaded record in current scope. Enabling history expands the scope to current plus historical. Selecting orbit `ALL` with type `DEBRIS` includes every accepted debris row in that scope. Selecting `GEO` and `MEO` shows only their union; selecting `LEO` shows only LEO records. The panel, search total, filtered count, and selected-state clearing must derive from the same normalized result set.
- Selecting type `DEBRIS` includes every accepted debris record in the chosen orbit/history scope regardless of small or missing RCS, and excludes separately classified rocket bodies. Object type never overwrites orbit class.
- Verify tag filters intersect without category-label collision. Only the exact orbit `ALL` plus object type `DEBRIS` workflow activates client-derived facets for position availability; reported RCS below 0.01, 0.01 to below 0.1, 0.1 to below 1, at least 1 m2, and not reported or invalid; owner/country; inclusive launch-year range; launch site; provider status; and international designator/tag. Facets must use unchanged tracked fields, compose by intersection inside that special workflow, and expose compact option controls. They must not appear for MEO/GEO/LEO/HRO/Other intersections or other object types. No mass or weight filter may appear. Share links round-trip orbit, object type, history, tag, every facet, search, and canonical selected identity; restoring the special facet state must also restore exact `ALL` plus `DEBRIS`. Legacy category/debris input remains bounded and cannot override explicit v2.3 state.
- Verify asynchronous object-type and history chunks load only when needed, reject external/escaping paths, cache canonical identities, and use a monotonically increasing generation. A delayed old request must be reported stale and must not replace the latest filter result.
- Counts separately report total, current tracked, historical tracked, matched/filtered, positioned/propagatable, metadata-only, render-ready, and quarantine. Manifest totals remain truthful while lazy chunks are pending; render-ready never exceeds propagatable.
- Metadata-only search/selection populates consolidated SATCAT details and an explicit no-current-elements state. An authoritative current tracked `NO_CURRENT_ELEMENTS` record must suppress an older same-NORAD GP proxy, mesh, selection, and screening entry rather than inheriting stale orbital state. Model, orbit, footprint, ground track, propagation, and screening controls remain unavailable. No placeholder at `(0,0,0)` may enter Globe, Mercator, or a Worker request.
- Verify tracked-object terminology replaces satellite-only wording on generic selection/count surfaces. The compact color key is payload cyan, debris red, rocket body amber, mission-related green, unknown gray, and selected white. Zero through 499 render-ready/drawn Globe objects must use the bundled same-origin `icons/ob_satellite.png` alpha silhouette at a fixed 16 screen pixels with `sizeAttenuation: false`; the shader must sample only the texture alpha so exact vertex colors remain authoritative. At 500 or more, the Globe must remove the map, restore `sizeAttenuation: true`, and use perspective-attenuated density size `0.025`. The procedural white-alpha circle may appear only after the asset fails to load. Both Globe paths remain color-only and make no per-type shape/ring claim. Detailed Mercator exercises type glyphs and selection rings through 1,000 drawable objects; above 1,000, dense Mercator is color-only. Only authoritative `DEBRIS` records with validated current elements may render red; red must never encode hazard, collision risk, proximity, physical size, mass, or RCS. In the frozen real snapshot, assert zero debris markers while all debris remains searchable. Deterministic fixtures with valid debris elements must exercise the red Globe and Mercator paths, including the exact 499/500 Globe boundary and large-set density path.
- Selected-object screening must show the manifest-backed metadata-only exclusion count and retain `tracked_count`, `propagatable_count`, `metadata_only_excluded_count`, and `screening_coverage_fraction` in request metadata and the frozen JSON export. Assert that current tracked equals propagatable plus excluded and that the ratio is labeled as input eligibility, not pair-screening completeness or collision probability. Delay or reject the manifest and require the denominator, excluded count, fraction, and visible summary to remain unknown; they must never fall back to zero excluded or 100%.

### API, scheduler, and static artifact

- Contract-test `/api/tracked-objects`, `/api/tracked-objects/manifest`, `/api/tracked-objects/chunks/{file_name}`, and the equivalent `server.py` `/json/tracked/` manifest, metadata, and chunk aliases. Only a current-manifest reference inside a coherent closure may return `200`; unsafe names, traversal encodings, and unreferenced chunks return `404` with no path disclosure.
- Corrupt, missing, mismatched, or escaping referenced chunks, malformed or one-sided pointers, manifest/metadata revision disagreement, raw GP/SATCAT bytes that do not match their metadata hashes, and stale GP/SATCAT revisions or accepted-source groups must make the entire tracked route family return bounded `503 TRACKED_CATALOG_UNAVAILABLE` as `application/problem+json`. Assert the problem instance is the requested path, health degrades, no private path appears, and the client stays on its GP-only fallback instead of accepting a partial tracked index. Replace files during validation and require either the exact validated manifest/metadata snapshot or `503`, never mixed generations.
- Verify live OpenAPI and static Swagger include the tracked `503`; verify MIME/cache/security headers, conditional reads, and response limits cover the tracked routes. The manifest alias and canonical route must return the same bytes when coherent.
- Verify `/api/data-update-status` exposes `tracked_pointer_valid`, tracked manifest/metadata revision agreement, current source-lineage agreement, counts, due state, effective independent interval, restart-persistent error, and the six-component composite revision. Changing only tracked bytes changes `data_revision`; metadata/no-change checks do not fabricate a new catalog revision.
- Verify every GP cycle requests `active`, `fengyun-1c-debris`, `iridium-33-debris`, and `cosmos-2251-debris` once. Incremental mode merges the newest validated epoch per exact identity. Reconciliation may replace only after all four configured responses are structurally valid and quarantine-free, and the resulting `COMPLETE` status applies only to that configured partial scope. One error, invalid response, quarantine, or unsafe conditional result must preserve the entire last-known-good catalog; it must never publish a partial group subset or claim all-debris/provider completeness.
- Verify active-only metadata migrates fail-closed. Until one successful four-group cycle records `source_groups` and `source_scope_verified: true`, all group requests are unconditional, the dataset remains due, and `304`-only responses cannot relabel the prior active-only bytes as the expanded scope. A failed migration records the attempted URLs and `source_scope_verified: false` while preserving data/revision bytes. The controlled live attempt at `2026-08-30T20:33:29Z` returned HTTP 503 and preserved the last-known-good snapshot; it is outage evidence, not a CI dependency or coverage result.
- `npm run serve` stays update-off. `npm run serve:update` and direct server startup accept `--tracked-update-interval-hours`, default it to the effective SATCAT interval, bind before catch-up, share one update lock/SATCAT fetch, rebuild tracked data locally, isolate failures, back off, recover stale locks, and stop cleanly.
- Static builds include `TRACKED.manifest.json`, `TRACKED.meta.json`, and only the manifest-referenced current/history/quarantine closure. Reject orphan retained chunks, missing references, descriptor/content mismatch, duplicate identity, partition drift, packaged GP byte/metadata drift, and tracked GP/SATCAT source-lineage disagreement. Build twice and require deterministic `asset-manifest.json` output.
- Static browser mode makes only same-origin catalog requests and refreshes on a tracked revision change without a provider request. Missing/invalid tracked data falls back visibly to the GP-only browser path; missing/invalid GP still follows the existing GP/TLE fail-closed contract.

### Motion and retained browser behavior

- Repeat the Version 2.2 signed-time regression with tracked filters active: selected-object propagation remains exact per frame, non-selected markers use bounded interpolation, rapid `Time x` changes cancel stale jobs, and motion is continuous rather than cadence-snapped at normal supported rates.
- Repeat forward, reverse, and paused `Time x` journeys on Globe and Mercator. Metadata-only records remain stationary details because they have no position; they must not inherit another object's state.
- In Solar System mode, `Time x` advances every planet from the bundled ephemeris, including forward future positions and inward reversal. Clamp and pause at the finite ephemeris boundary; never extrapolate.
- Verify popup/layer ordering, selected details, Mercator interaction, timelines, server controls, canvas pixels, accessibility, responsive layout, and zero unexpected page/console/request errors remain intact with lazy tracked loading.

### Scale, failure, and delivery gates

- Generate at least 120,000 deterministic records spanning orbit classes, types, current/history, small/missing RCS, long IDs, and metadata-only availability. Bound fixture construction, a multi-dimension filter, bounded search, facet construction, total elapsed time, and heap growth. The authoritative frontend observation measured 30.82 ms build, 154.15 ms filter, 51.28 ms search, 228.97 ms facets, 465.23 ms total, and 77.10 MiB heap growth. Record these as a development regression observation, not a portable percentile baseline.
- Exercise rapid filter races, repeated toggles, zero-result recovery, deferred history, failed chunk reload with last-known-good preservation, malformed records beside valid records, and selected-object exclusion while the simulation clock is running.
- Run the frozen real-snapshot invariants, focused backend/API/scheduler/security matrix, complete JavaScript and Python suites, browser matrix, static build, asset budgets, version/vendor checks, validation manifest check, dependency audit, and SBOM generation before delivery.
- Bind the exact Version 2.3.1 manifest/chunks/quarantine/static/source hashes to the final post-recording publication seal. The named owner approval covers only that one `origin/master` and repository-root GitHub Pages publication; every later changed provider or generated byte returns to the redistribution gate. A passing test suite is not provider-completeness evidence, scientific validation, or operational authorization.

### Required Version 2.3.1 commands
...
Final post-hardening local verification passed 59 of 59 JavaScript unit test files. Python discovered 129 cases and completed in 32.119 seconds with 128 passes, one intentional Windows directory-symlink capability skip, and zero failures. The complete 47-declaration Playwright matrix finished in exactly 532467.509 ms (8:52.468, or 8.874458483 minutes) with 28 passes, 19 explicit-reason intentional skips, zero unexpected failures, zero flaky results, zero report errors, and one attempt each. Syntax validation covered 136 JavaScript files, dependency audit reported zero vulnerabilities, and the asset-budget check passed the reviewed Version 2.3.1 ceilings. The final fixed-point static build contains 135 files totaling 280,508,022 bytes. The strict Version 2.3.1 corpus/check passes and binds 307 artifacts, 13 executable declarations, and 64 evidence records. The completed Version 2.3.0 figures remain historical evidence and must not be relabeled as 2.3.1 results.

## Historical Version 2.2 GP/OMM Development Verification
...
## Historical Version 2.2 Browser-Correctness Follow-up Verification
...
- Verify the Globe uses the loaded `icons/ob_satellite.png` alpha silhouette at fixed 16-screen-pixel size without attenuation for 0 through 499 drawn objects, preserving the canonical vertex color while retaining transparent artwork detail. At exactly 500, verify the map is absent, point size is `0.025`, and perspective attenuation is restored. Force an icon-load failure and verify only that path substitutes the procedural white-alpha circle. This Globe boundary is independent of Mercator, which remains detailed through 1,000 drawable objects and switches to density only above 1,000.
...
- In the mobile conjunction journey, fulfill the ISS model request with the checked-in lightweight `obj/starlink_v2.glb` fixture so detailed-model loading remains covered without transferring the production ISS payload. Its `180 s` whole-test timeout is hosted-runner infrastructure allowance, not performance evidence.
...
- Test `/api/data-update-status` returns scheduler state; six-component server `data_revision`; compatibility GP-only `catalog_revision`; GP/TLE/SATCAT/tracked/launch/decay top-level and nested revisions; intervals, retry/next-check, persistent dataset state/errors after restart, reconciliation time, and disabled/default-off behavior unless scheduling is explicitly enabled. Recursively verify nested result errors are bounded and redact credentials/control characters. Verify `fallback-tle` requires absent GP metadata plus a packaged TLE file larger than the empty `[]` sentinel, that missing/empty inputs report `unavailable`, and that this availability signal is not presented as TLE validation.
...
- `/api/gp-metadata` matches the packaged GP sidecar, tracked manifest/chunk APIs expose only verified current references, and `/api/data-update-status` exposes the composite plus all six GP/TLE/SATCAT/tracked/launch/decay component revisions with a truthful `catalog_state`.
